...
public class TransactionProcessorTest {
  ...
  public void testPreExistingData() throws Exception {
    ...
    byte[] familyBytes = Bytes.toBytes("f");
    long ttl = 6000;
    HRegion region = createRegion(tableName, familyBytes, ttl);
    try {
      region.initialize();

      // timestamps for pre-existing, non-transactional data
      long now = txSnapshot.getVisibilityUpperBound() / TxConstants.MAX_TX_PER_MS;
      long older = now - ttl / 2;
      long newer = now - ttl / 3;
      // timestamps for transactional data
      long nowTx = txSnapshot.getVisibilityUpperBound();
      long olderTx = nowTx - (ttl / 2) * TxConstants.MAX_TX_PER_MS;
      long newerTx = nowTx - (ttl / 3) * TxConstants.MAX_TX_PER_MS;

      Map<byte[], Long> ttls = Maps.newTreeMap(Bytes.BYTES_COMPARATOR);
      ttls.put(familyBytes, ttl);
      ...
      txScan.setFilter(new TransactionVisibilityFilter(
        TxUtils.createDummyTransaction(txSnapshot), ttls, false, ScanType.USER_SCAN));
      ...
      // verify get
      getAndAssert(region, cells, cells.get(0).getRow(), cells.get(2).getRow(), cells.get(4).getRow());
      ...
      // Reduce ttl, this should make cells with timestamps older or olderTx expire
      long newTtl = ttl / 2 - 1;
      ...
      scanAndAssert(region, cells, rawScan);
      // However, tx scan and tx get should not-return expired cells
      scanAndAssert(region, select(cells, 1, 3, 5), txScan);
      getAndAssert(region, select(cells, 1, 3, 5), cells.get(0).getRow(), cells.get(2).getRow(), cells.get(4).getRow());
  ...
  private void getAndAssert(HRegion region, List<Cell> expected, byte[]... rows) throws Exception {
    List<Cell> actual = new ArrayList<>();
    for (byte[] row : rows) {
      Get get = new Get(row);
      get.setAttribute(TxConstants.TX_OPERATION_ATTRIBUTE_KEY,
                       new TransactionCodec().encode(TxUtils.createDummyTransaction(txSnapshot)));
      Result result = region.get(get);
      if (!result.isEmpty()) {
        actual.addAll(result.listCells());
      }
    }
